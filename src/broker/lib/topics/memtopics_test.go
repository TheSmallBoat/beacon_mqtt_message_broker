...
func TestRNodeInsertRemove(t *testing.T) {
	...
	err := n.rinsert([]byte(msg.TopicName), msg)
	...
	require.Equal(t, msg2.TopicName, n6.msg.TopicName)
...
func TestRNodeMatch(t *testing.T) {
	...
	err := n.rinsert([]byte(msg1.TopicName), msg1)
	require.NoError(t, err)
	...
	err = n.rinsert([]byte(msg3.TopicName), msg3)
	require.NoError(t, err)

	var msglist []*packets.PublishPacket
	// ---
	err = n.rmatch([]byte(msg1.TopicName), &msglist)
	require.NoError(t, err)
	require.Equal(t, 1, len(msglist))

	// ---
	msglist = msglist[0:0]
	err = n.rmatch([]byte(msg2.TopicName), &msglist)
	require.NoError(t, err)
	require.Equal(t, 1, len(msglist))

	// ---
	msglist = msglist[0:0]
	err = n.rmatch([]byte(msg3.TopicName), &msglist)
	require.NoError(t, err)
	require.Equal(t, 1, len(msglist))
...
func TestMemTopicsSubscription(t *testing.T) {
	...
	err = mgr.Subscribers([]byte("sports/tennis/anzel/stats"), QosExactlyOnce, &subs, &qoss)
	require.NoError(t, err)
	require.Equal(t, 1, len(subs))
	require.Equal(t, 2, int(qoss[0]))

	err = mgr.Subscribers([]byte("sports/tennis/anzel/stats"), QosAtLeastOnce, &subs, &qoss)
	require.NoError(t, err)
	require.Equal(t, 1, len(subs))
	require.Equal(t, 1, int(qoss[0]))
