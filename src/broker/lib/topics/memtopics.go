...
func (this *memTopics) Subscribe(topic []byte, qos byte, sub interface{}) (byte, error) {
	if !ValidQos(qos) {
		return QosFailure, fmt.Errorf("memtopics/Subscribe: Invalid QoS %d", qos)
	}

	if sub == nil {
		return QosFailure, fmt.Errorf("memtopics/Subscribe: Subscriber cannot be nil")
	}
...
func (this *memTopics) Subscribers(topic []byte, qos byte, subs *[]interface{}, qoss *[]byte) error {
	if !ValidQos(qos) {
		return fmt.Errorf("memtopics/Subscribers: Invalid QoS %d", qos)
	}
