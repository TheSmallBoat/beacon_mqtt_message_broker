...
func SubscribeTopicSpilt(topic string) ([]string, error) {
	...
	if bytes.IndexByte(subject, '#') != -1 {
		if bytes.IndexByte(subject, '#') != len(subject)-1 {
			return nil, errors.New("acl/SubscribeTopicSpilt: Topic format error with index of #")
		}
	}
...
func PublishTopicSpilt(topic string) ([]string, error) {
	...
	if bytes.IndexByte(subject, '#') != -1 || bytes.IndexByte(subject, '+') != -1 {
		return nil, errors.New("acl/PublishTopicSpilt: Publish Topic format error with + and #")
	}
	re := strings.Split(topic, "/")
	for i, v := range re {
		if v == "" {
			if i != 0 && i != (len(re)-1) {
				return nil, errors.New("acl/PublishTopicSpilt: Topic format error with index of //")
			} else {
				re[i] = "/"
			}
		}
